...
const SUGGESTION_TYPE_COLORS = {
  aahar: "#16a34a", vihar: "#2563eb", aushadh: "#7c3aed",
  nidra: "#7c3aed", vihara: "#2563eb", charya: "#f59e0b",
  manas: "#c026d3", ojas: "#0f766e",
};
...
function TrendLine({ vals = [], labels = [], color = C.g3, h = 100 }) {
  if (vals.length < 2) {
    return (
      <div style={{
        height: h, display: "grid", placeItems: "center", color: C.muted, fontSize: 10,
...
export default function DashboardWellness() {
  ...
  const { data: wellnessIndexData, status: wellnessIndexStatus } =
    useSelector((s) => s.wellnessIndex);

  const { status: moodStatus, submittedScore } = useSelector((s) => s.wellnessMood);
  const moodLoggedToday = useSelector(selectMoodLoggedToday);
    ...
    <Box sx={{
      ...
      <ClientCard style={{ background: "rgba(107,179,63,0.04)" }} borderColor="rgba(107,179,63,0.18)">
        ...
        {suggestionsStatus === "loading" && (
          <div style={{ fontSize: 10, color: C.muted, padding: "8px 0" }}>Loading lifestyle suggestions…</div>
        )}
                  ...
                  <div style={{
                    ...
                    {isActioned ? (
                      <span style={{ fontSize: 9, fontWeight: 700, color: C.g3, flexShrink: 0 }}>
                        ...
                        {item.action === "skipped" && "✕ Skipped"}
                        {item.action === "saved" && "🔖 Saved"}
                      </span>
                    ) : (
