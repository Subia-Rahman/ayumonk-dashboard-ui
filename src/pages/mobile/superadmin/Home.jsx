...
const QUICK_ACTIONS = [
  { i: "🏢", l: "Add Company", c: C.blue, path: "/super-admin/company-data/add" },
  { i: "👤", l: "Add User", c: C.g3, path: "/super-admin/company-users/add" },
  { i: "🎯", l: "New KPI", c: C.gold, path: "/super-admin/kpis/add" },
  { i: "❓", l: "Add Question", c: C.orange, path: "/super-admin/questions/add" },
  { i: "🌿", l: "Suggestions", c: C.teal, path: "/super-admin/suggestion-master" },
  { i: "🏆", l: "Challenges", c: C.purple, path: "/super-admin/challenges" },
];
...
export default function SaHome() {
  ...
  const stats = useMemo(() => {
    ...
    const userLabel =
      totalUsers >= 1000 ? `${(totalUsers / 1000).toFixed(1)}K` : String(totalUsers);
    return {
      companies: total,
      users: userLabel,
    };
  ...
  return (
    <div>
      ...
      >
        ...
        <div
          style={{
            ...
            height: 32,
            borderRadius: 10,
            background: `linear-gradient(135deg,${C.purple}88,${C.purple})`,
            display: "flex",
            alignItems: "center",
      ...
      <div style={{ padding: "2px 16px 10px" }}>
        <div style={{ fontSize: 9, color: C.muted }}>Platform overview</div>
      ...
      >
        {[
          {
            l: "Companies",
            v: companiesLoading && !stats.companies ? "…" : stats.companies,
            i: "🏢",
            c: C.blue,
            d: "",
          },
          { l: "Total Users", v: stats.users, i: "👥", c: C.g3, d: "" },
          { l: "Active KPIs", v: "284", i: "🎯", c: C.gold, d: "across all cos" },
          { l: "Platform WI", v: "71.2", i: "📈", c: C.teal, d: "▲ 2.1% MoM" },
        ].map((s) => (
          <div
            key={s.l}
            style={{
          ...
          >
            ...
            <div style={{ fontSize: 21, fontWeight: 800, color: s.c, lineHeight: 1 }}>
              {s.v}
            </div>
            <div style={{ fontSize: 9, color: C.muted, marginTop: 1 }}>{s.l}</div>
            {s.d && (
              <div
                style={{ fontSize: 8, color: "rgba(255,255,255,.25)", marginTop: 3 }}
              >
                {s.d}
              </div>
            )}
        ...
        {recent.map((c) => (
          ...
          >
            ...
            <div style={{ textAlign: "right" }}>
              ...
              <div style={{ fontSize: 8, color: C.muted }}>Avg WI</div>
            </div>
          </div>
        ))}
      </div>
