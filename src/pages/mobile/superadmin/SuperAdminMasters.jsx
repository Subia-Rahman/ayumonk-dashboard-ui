...
function injectSaCss(){
  ...
  // Alignment overrides for all super-admin bottom-sheets / dialogs: anchor to
  // the viewport, stay above the bottom nav, and keep content padded above the
  // home indicator so the last field never hides behind the tab bar.
  const fix = document.createElement("style");
  fix.dataset.ayumonkSaFix = "true";
  fix.textContent =
    ".sa-sheet-wrap,.sa-drawer-wrap{position:fixed;z-index:300;}" +
    ".sa-sheet{max-height:86vh;border-radius:22px 22px 0 0;overflow:hidden;}" +
    ".sa-sheet-body{padding:16px 18px 10px;}" +
    ".sa-sheet-foot{padding:12px 18px calc(20px + env(safe-area-inset-bottom));}" +
    ".sa-screen{padding-bottom:28px;}";
